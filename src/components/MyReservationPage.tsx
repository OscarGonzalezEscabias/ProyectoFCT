...
import ProfileActivitiesReservationCard from "./cards/ProfileActivitiesReservationCard";
import ProfileFlightsReservationCard from "./cards/ProfileFlightsReservationCard";
import ProfileHotelsReservationCard from "./cards/ProfileHotelsReservationCard";
import ProfileTravelReservationCard from "./cards/ProfileTravelReservationCard";

type ReservationType = 'all' | 'hotels' | 'flights' | 'activities' | 'travels';
...
export default function MyReservationsPage({ params }: { params: { id: number } }) {
  ...
  const fetchReservations = async (type: ReservationType) => {
    ...
    try {
      ...
      if (type === 'all' || type === 'travels') {
        const res = await fetch(`/api/profile/${params.id}/travels-reservation`);
        if (res.ok) {
          const data = await res.json();
          allData = [...allData, ...data.map((item: any) => ({ ...item, type: 'travel' }))];
        }
      }
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {reservations.map((reservation, index) => {
            switch (reservation.type) {
              ...
              case 'travel':
                return <ProfileTravelReservationCard key={index} reservation={reservation} />;
              default:
                return null;
            }
          })}
